...
# Resolver IP de los contenedores vía DNS o /etc/hosts
PG_HOST=$(getent hosts postgres-prod.svc.local | awk '{ print $1 }')
MYSQL_HOST=$(getent hosts dev-mysql.dev.local | awk '{ print $1 }')
...
mysqldump --no-tablespaces -h "$MYSQL_HOST" -u drupal -pdrupal drupal > "$BACKUP_DIR/mysql_backup_$FECHA.sql"
