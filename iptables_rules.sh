...
iptables-legacy -t nat -A POSTROUTING -o eth4 -j MASQUERADE
...
# 🔓 Permitir tráfico saliente hacia Internet a través de eth4
...
# 🔓 Permitir tráfico saliente hacia Internet a través de eth4
iptables-legacy -A FORWARD -s 172.20.0.0/24 -o eth4 -j ACCEPT
iptables-legacy -A FORWARD -s 172.30.0.0/24 -o eth4 -j ACCEPT
iptables-legacy -A FORWARD -s 172.40.0.0/24 -o eth4 -j ACCEPT

# 🔁 Permitir tráfico de respuesta
iptables-legacy -A FORWARD -d 172.20.0.0/24 -m state --state ESTABLISHED,RELATED -j ACCEPT
iptables-legacy -A FORWARD -d 172.30.0.0/24 -m state --state ESTABLISHED,RELATED -j ACCEPT
iptables-legacy -A FORWARD -d 172.40.0.0/24 -m state --state ESTABLISHED,RELATED -j ACCEPT
